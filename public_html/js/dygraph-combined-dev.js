...
//            console.log(e.dygraph.pqcolor[idx][e.pqmcolor])
            var cb = pointsOnLine[idx];
            ctx.save();
            //console.log(e.dygraph.pqcolor[idx][e.pqmcolor]);
            drawPointCallback.call(e.dygraph,
                    e.dygraph, e.setName, ctx, cb[0], cb[1], e.dygraph.pqcolor[idx][e.pqmcolor], pointSize, cb[2]);
            ctx.restore();
        }
    };
    ...
    DygraphCanvasRenderer._linePlotter = function (e) {
        ...
        var drawPointCallback = g.getOption("drawPointCallback", setName) ||
                Dygraph.Impulse.DEFAULT;//Circles.DEFAULT;
...
var Dygraph = (function () {
     ...
     */
    var Dygraph = function (div, data, color, mupdate, gid, opts, opt_fourth_param) {
...
// TODO(danvk): use these functions throughout dygraphs.
    ...
    Dygraph.prototype.parseArray_ = function (data) {
        ...
        if (this.attr_("labels") === null) {
            ...
            if (num_labels.length != num_labels.length) {
    ...
    Dygraph.prototype.updateColourOptions = function (color) {
        this.pqcolor.push(color);
        this.pqcolor.shift();
    }
...
// A whitelist of options that do not change pixel positions.
    ...
    Dygraph.Impulse = {
        DEFAULT: function (g, name, ctx, canvasx, canvasy, color, radius) {
            ...
            ctx.lineWidth = "1"
            ctx.strokeStyle = color;
            ctx.moveTo(canvasx, this.height_);
            ctx.lineTo(canvasx, canvasy);
            //ctx.arc(canvasx, 180, radius, 0, 2 * Math.PI, false);
            ctx.stroke();
        }
        // For more shapes, include extras/shapes.js
    };
...
(function () {
    ...
    Dygraph.Interaction.startPan = function (event, g, context) {
        ...
        for (i = 0; i < g.axes_.length; i++) {
            ...
            if (axis.valueWindow || axis.valueRange)
...
// Needed for dashed lines.
    ...
    legend.prototype.select = function (e) {
        ...
        if (e.dygraph.pqmuseup) {
            e.dygraph.pqmuseup(xValue, e.dygraph.pqgraphid);
        }
    };
...
//------------------------------------------------------------------
    ...
    rangeSelector.prototype.computeCombinedSeriesAndLimits_ = function () {
        ...
        for (i = 1; i < numColumns; i++) {
            ...
            if (include !== null)
                anySet = true;  // it's set explicitly for this series
        }
        if (!anySet) {
...
(function () {
    ...
    for (var k in Dygraph.OPTIONS_REFERENCE) {
        if (!Dygraph.OPTIONS_REFERENCE.hasOwnProperty(k))
            continue;
