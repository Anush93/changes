...
//        minDate: device_set_time,
    ...
    $("#billing_month_select").on("dp.change", function (e) {
        var bill_time = new Date($('#billing_month_select').data("DateTimePicker").date());        
        var year = moment(bill_time).format('YYYY');
        var month = moment(bill_time).format('MM');
        Update_Billing_Data(parseInt(year), parseInt(month));
    });        
        $('#billing_month_select').data('DateTimePicker').date(moment(Date.now()).format('MMM YYYY'));
    });
};
